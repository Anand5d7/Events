...
class Events extends Component {
  state = {activeEventsId: ''}

  getActiveEventRegistrationStatus = () => {
    const {activeEventsId} = this.state
    const activeEventDetails = eventsList.find(
      event => event.id === activeEventsId,
    )
  ...
  setActiveEventId = id => {
    this.setState({activeEventsId: id})
  }
        ...
        <div className="events-container">
          ...
          <ul className="list-container">
            {eventsList.map(eachEvent => (
              <EventItem
                key={eachEvent.id}
                eventsData={eachEvent}
                setActiveEventId={this.setActiveEventId}
                isActive={eachEvent.id === activeEventsId}
              />
            ))}
          </ul>
        </div>
